...
;;; -----------------------------------------------------------------------------
;;;
;;; This file implements the psi(digamma) functions.  It is based on the Special
;;; Functions in the GNU Scientific Library (GSL).
;;;
;;; Version  Date      Description
;;; 1.0.0    09/28/04  Marked as ready from Release 1.0.  Added contracts for
;;;                    functions. (MDW)
...
(define (psi-x x)
  (let ((y (abs x)))
    (cond ((or (= x 0.0)
          ...
          ((>= y 2.0)
           (let* ((t (- (/ 8.0 (* y y)) 1.0))
                  (result-c-val (unchecked-chebyshev-eval apsi-cs t)))
             (if (< x 0.0)
                 (let ((s (sin (* pi x)))
                       (c (cos (* pi x))))
                   (when (< (abs s) (* 2.0 sqrt-double-min))
                     +nan.0)
                   (+ (log y)
                      (/ -0.5 x)
                      result-c-val
          ...
          (else
           ...
           (cond ((< x -1)
                  ...
                  (let* ((v (+ x 2.0))
                         ...
                         (t3 (/ 1.0 x))
                         (result-c-val
                          (unchecked-chebyshev-eval psi-cs (- (* 2.0 v) 1.0))))
                 ...
                 ((< x 1.0)
                  ...
                  (let* ((t1 (/ 1.0 x))
                         (result-c-val
                          (unchecked-chebyshev-eval psi-cs (- (* 2.0 x) 1.0))))
                    (+ (- t1) result-c-val)))
                 (else
                  ;; x = 1 + v
                  (let ((v (- x 1)))
           ...
           (let* ((y2 (* ay ay))
                  ...
                  (v (* y2
                        (+ (/ 1.0 (+ 1.0 y2))
                           (/ 0.5 (+ 4.0 y2)))))
                  (result-c-val
                   (unchecked-chebyshev-eval r1py-cs x)))
